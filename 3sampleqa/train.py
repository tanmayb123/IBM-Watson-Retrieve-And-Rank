...
with open(RELEVANCE_FILE, 'rb') as csvfile:
    ...
    with open(TRAININGDATA, "a") as training_file:
        ...
        for row in question_relevance:
    	    question = row[0]
            shouldEndWhile = 0
            while shouldEndWhile < 10:
                relevance = ','.join(row[1:])
                curl_cmd = 'curl -k -s -u %s -d "q=%s&gt=%s&generateHeader=%s&rows=%s&returnRSInput=true&wt=json" "%s"' % (CREDS, question, relevance, add_header, ROWS, SOLRURL)
                ...
                try:
                    ...
                    print ('Response:')
                    print (output)
                    shouldEndWhile = shouldEndWhile + 1
            add_header = 'false'
